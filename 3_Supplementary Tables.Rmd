...
tbl_strata_ex1 <-
  ...
  mutate(educ_hh2= case_when(
    str_starts(educ_hh, "Primary") ~ "Primary Education",
    str_starts(educ_hh, "Secondary") ~ "Secondary Education",
    str_starts(educ_hh, "University") ~ "University Education",
    str_starts(educ_hh, "Postgraduate|Doctorate") ~ "University Education",
    TRUE ~ educ_hh
  ))%>%
  ...
  sjlabelled::var_labels(educ_hh2="Education level", num_csa="Number of CSA adopted")%>%
  tbl_summary(
    by = country,
...
fig_1 <- ggplot(df, aes(y=value, x=gap, color=name2)) + 
...
practices. Loess smoothing is a nonparametric method that uses local weighted regression to fit a smooth curve through points in a scatter plot. This line represents the trend of CSA adoption as aspiration gap changes. The shaded region around the Loess line represents 95\% Confidence intervals. The width of the shaded region at any point along the x-axis provides a measure of the uncertainty around the estimate of the mean of adoption rate at that point. 
}
\end{figure}
...
# Calculate the percentiles
percentiles <- quantile(data, probs = c(0.75, 0.95))

# Create a data frame for the percentiles
percentile_df <- data.frame(
  Percentile = c("75th", "95th"),
  Value = percentiles
)
...
# df 2 group by name2 and get the 75th and 90th percentile of gap
df2 <- df %>%
  group_by(name2) %>%
  summarise(percentile_75 = quantile(gap, 0.75),
            percentile_90 = quantile(gap, 0.90))
...
fig_2 <- ggplot(df_cam, aes(y=value, x=gap, color=name2)) + 
  geom_smooth(method="loess", se=TRUE, span=0.9) +
  facet_wrap(~name2, scales = "free_y") + # use facet_grid(~csa, scales = "free") for a grid layout
  scale_color_brewer(palette="Set1")+ 
  labs(x = "Aspiration gap", y = "") +
...
\caption*{
Note: The graph illustrates the relationship between the variable CSA and Aspiration Gap, stratified by the four different practices for Cameroon.The solid line on each plot depicts a Loess smoothed relationship between aspiration gap and CSA practices. Loess smoothing is a nonparametric method that uses local weighted regression to fit a smooth curve through points in a scatter plot. This line represents the trend of CSA adoption as aspiration gap changes. The shaded region around the Loess line represents 95\% Confidence intervals. The width of the shaded region at any point along the x-axis provides a measure of the uncertainty around the estimate of the mean of adopion rate at that point. 
}
\end{figure}
...
fig_3 <- ggplot(df_ken, aes(y=value, x=gap, color=name2)) + 
  geom_smooth(method="loess", se=TRUE, span=0.9 ) +
  facet_wrap(~name2, scales = "free_y") + # use facet_grid(~csa, scales = "free") for a grid layout
  scale_color_brewer(palette="Set1")+ 
  labs(x = "Aspiration gap", y = "") +
...
library(KernSmooth)
library(locpol)
y <-aspiration_df$intercropping
x <-aspiration_df$gap
plot(x, y)
fit <- locpoly(x, y, bandwidth = 0.3)
plot(fit)

rr <- locpol::locpol(intercropping ~ gap, data = aspiration_df, bw = 0.13)
plot(rr)
